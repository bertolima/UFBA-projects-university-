	...
	def verificar_fim_jogo(self):
		...
		for jogador in self.jogadores:
			if len(jogador.getCartas())>0:
				if str(jogador.getCartas()[0])[:-1]==str(self.mico.__dado__)[:-1]:
					CartaPerdedora = jogador.getCartas()[0]
					return (f"Como o mico era {str(self.mico.__dado__)},então {jogador.nome} perdeu, pois estava com a carta {CartaPerdedora}!!!")
